...
## 4面立面図が1枚に印刷されている場合（最重要）
日本の住宅図面は「南立面図・北立面図・東立面図・西立面図」が1枚に印刷されることが多い。
その場合：
- 各立面図には「南立面図」「北立面図」などのラベルが付いている → そのラベルで面を特定する
- ラベルがない場合は位置で判断（左上=南、右上=北、左下=東、右下=西 が一般的）
...
- 窓・ドア・玄関などの開口部は各面ごとにリストアップし、幅×高さ×位置（m）を記録すること
